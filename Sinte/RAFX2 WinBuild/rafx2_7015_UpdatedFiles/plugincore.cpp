...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::waveform, "Waveform", "sine,square,triangle,sawtooth", "sine");
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	{
		y = Amplitud * sin(fase);
		fase = fase + (2 * pi * frecuenciaUsuario) / audioProcDescriptor.sampleRate;
		if (fase > 2 * pi)
		{
			fase = fase - (2 * pi);
		}
	...
	if (getPluginType() == kSynthPlugin)
	{
		...
		processFrameInfo.audioOutputFrame[0] = y;
		if (processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
			processFrameInfo.audioOutputFrame[1] = y;
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::Frecuencia_Hz, 0.000000);
	addPreset(preset);
